...
# 拟合下边界
lower_bound_parameter = np.polyfit(lower_bound_y, lower_bound_x, 2)
# 存储拟合的下边界的坐标
y_lower_bound = []
try:
    for i in range(0, cols):
        y = int(lower_bound_parameter[0]*i**2 + lower_bound_parameter[1]*i + lower_bound_parameter[2])
...
blur = cv2.GaussianBlur(img4,(3,3),0)
