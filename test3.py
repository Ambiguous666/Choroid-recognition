...
for x in range(0,rows):
    num = 0
    for y in range(0,cols):
        if img_final_array[x,y]>0:
            num+=1
    if num<30:
        for z in range(0,cols):
            img_final_array[x,z] = 0
# img3 = Image.fromarray(img_final_array)
# img3.show()
...
for x in range(0,cols):
    for y in range(0,rows):
        if y < y_lower_bound[x][0]+38 or y > y_lower_bound[x][0]+100:
...
nihe_line = []
nihe_parameter = np.polyfit(nihe_x,nihe_y,2)
for i in range(0,cols):
    y = int(
        nihe_parameter[0] * i ** 2 + nihe_parameter[1] * i + nihe_parameter[2])
    nihe_line.append([i,y])

for x in range(0,cols):
    for y in range(0,rows):
        if y<nihe_line[x][1] - 10 or y>nihe_line[x][1] + 15:
            img_final_array[y,x] = 0
# img3 = Image.fromarray(img_final_array)
# img3.show()
...
for x in range(0,cols):
    for y in range(0,rows):
        if img_final_array[y,x]>0:
            nihe2_x.append(x)
            nihe2_y.append(y)
nihe2_parameter = np.polyfit(nihe2_x,nihe2_y,3)
for i in range(0,cols):
    y = int(nihe2_parameter[0] * i ** 3 + nihe2_parameter[1] * i ** 2 + nihe2_parameter[2] * i + nihe2_parameter[3])
    img_rgb_array[y,i] = [0, 255, 255]
img_rgb_final = Image.fromarray(img_rgb_array)
img_rgb_final.show()
