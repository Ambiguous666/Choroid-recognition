...
path = 'E:/LearningProjects/OCT脉络膜识别/HD/HDimage086.png'  # 图97算是比较标准
...
# 将图中区域分块
different = int((mylist[mylist_len-1][0] - mylist[0][0])/3)
...
parameter = np.polyfit(mylist_zone3_y,mylist_zone3_x,3)
# y1 = parameter[0]*x**3 + parameter[1]*x**2 + parameter[2]*x + parameter[3]

# 绘制曲线
mylist_zone3_len = len(mylist_zone3)
for i in range(0,cols):
    y = int(parameter[0]*i**3 + parameter[1]*i**2 + parameter[2]*i + parameter[3])
