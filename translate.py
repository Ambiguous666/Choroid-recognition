...
path = 'E:/LearningProjects/OCT脉络膜识别/HD/HDimage103.png'
...
for x in range(0,cols):
    for y in range(0,rows):
        if img_final_array[y,x]>0:
            nihe2_x.append(x)
            nihe2_y.append(y)
nihe2_parameter = np.polyfit(nihe2_x,nihe2_y,4)
for i in range(0,cols):
    y = int(nihe2_parameter[0] * i ** 4 + nihe2_parameter[1] * i ** 3 + nihe2_parameter[2] * i ** 2 + nihe2_parameter[3] * i +
        nihe2_parameter[4])
